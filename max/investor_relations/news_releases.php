...
<TR><TD align=right><B>Date</B></TD><TD><B>Title</B> <FONT size=-1>(click to view the full text)</FONT></TD></TR>
<?php
	...
	foreach ($NewsReleases as $NewsRelease) {
		...
		if ( (strtotime('Feb 04 2002') < $pressTimeStamp) || $_GET['archive'] ) {
			...
			<TD><A HREF=\"news_releases_display.php?REQID=$ReqID\" class=\"interior\">$title</TD></TR>";
		}
	}
	print "<TR><TD colspan=2 align=center><H3><BR>";
	if ($_GET['archive']) {
		print '<A HREF="news_releases.php">Current</A> | Archive';
	}
	else {
		print 'Current | <A HREF="news_releases.php?archive=1">Archive</A>';
	}
	print "</H3></TD></TR>";
?>
</TABLE>
